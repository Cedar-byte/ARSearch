...
function Marker(poiData) {
    ...
    /*
      With AR.PropertyAnimations you are able to animate almost any property of ARchitect objects.
       This sample will animate the opacity of both background drawables so that one will fade out while the other one fades in.
        The scaling is animated too.
        The marker size changes over time so the labels need to be animated too in order to keep them relative to the background drawable.
    ...
    */
    ...
    // create an AR.ImageDrawable for the marker in idle state ==== offsetY 这个是偏移量，防止标签在同一水平线上
    this.markerDrawable_idle = new AR.ImageDrawable(World.markerDrawable_idle, 2.5, {
        zOrder: 0,
        opacity: 1.0,
        offsetY: -5.55,
        /*
          To react on user interaction,
          an onClick property can be set for each AR.Drawable.
          ...
          The function returns a function which checks the selected state with the help of the variable isSelected and executes the appropriate function.
          The clicked marker is passed as an argument.
        */
...
//        offsetY: -0.55,
    ...
    /*
        方向指示器
        Create an AR.ImageDrawable using the AR.
        ImageResource for the direction indicator which was created in the World.
...
/*
    ...
    更改 hideIdleDrawableAnimation(opacity 改为1.0) 和 showSelectedDrawableAnimation(opacity 改为0.0) 参数中的透明值、
    这样当点击Marker时不会出现selected drawable图片资源
    Property Animations allow constant changes to a numeric value/property of an object,
    dependent on start-value, end-value and the duration of the animation.
...
Marker.prototype.setSelected = function(marker) {
    ...
    if (marker.animationGroup_selected === null) {
        ...
        var hideIdleDrawableAnimation = new AR.PropertyAnimation(marker.markerDrawable_idle, "opacity", null, 1.0, kMarker_AnimationDuration_ChangeDrawable);
        // create AR.PropertyAnimation that animates the opacity to 1.0 in order to show the selected-state-drawable
        var showSelectedDrawableAnimation = new AR.PropertyAnimation(marker.markerDrawable_selected, "opacity", null, 0.0, kMarker_AnimationDuration_ChangeDrawable);
    ...
    // enables the direction indicator drawable for the current marker  是否显示指示器图标
...
Marker.prototype.setDeselected = function(marker) {
    ...
    if (marker.animationGroup_idle === null) {
        ...
        // 标题的动画 create AR.PropertyAnimation that animates the scaling of the title label to 1.0
        ...
        // 内容的动画create AR.PropertyAnimation that animates the scaling of the description label to 1.0
    ...
    // disables the direction indicator drawable for the current marker 是否显示指示器图标
